...
-- Works in mysql client and JDBC script runners (no DELIMITER / stored procedure needed)

SET @sql = NULL;
SELECT CASE WHEN (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = DATABASE() AND table_name='students' AND column_name='program') = 0
            THEN 'ALTER TABLE students ADD COLUMN program VARCHAR(200) AFTER full_name'
            ELSE 'SELECT 1'
       END INTO @sql;
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SELECT CASE WHEN (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = DATABASE() AND table_name='students' AND column_name='department') = 0
            THEN 'ALTER TABLE students ADD COLUMN department VARCHAR(100) AFTER program'
            ELSE 'SELECT 1'
       END INTO @sql;
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SELECT CASE WHEN (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = DATABASE() AND table_name='students' AND column_name='enrollment_date') = 0
            THEN 'ALTER TABLE students ADD COLUMN enrollment_date DATETIME DEFAULT CURRENT_TIMESTAMP'
            ELSE 'SELECT 1'
       END INTO @sql;
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SELECT CASE WHEN (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = DATABASE() AND table_name='quizzes' AND column_name='enrollment_required') = 0
            THEN 'ALTER TABLE quizzes ADD COLUMN enrollment_required BOOLEAN DEFAULT TRUE'
            ELSE 'SELECT 1'
       END INTO @sql;
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SELECT CASE WHEN (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = DATABASE() AND table_name='quizzes' AND column_name='description') = 0
            THEN 'ALTER TABLE quizzes ADD COLUMN description TEXT'
            ELSE 'SELECT 1'
       END INTO @sql;
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
